...
Function Parse-and-Send($qualysResponse){
	...
	$qualysResponse.HOST_LIST_VM_DETECTION_OUTPUT.RESPONSE.HOST_LIST.HOST | ForEach-Object {
		...
		Add-Member -InputObject $hostObject -MemberType NoteProperty -Name "Tags" -Value $_.TAGS
		...
		foreach($detection in $_.DETECTION_LIST.DETECTION){
			...
			Add-Member -InputObject $detectionObject -MemberType NoteProperty -Name "TITLE" -Value $detection.TITLE
